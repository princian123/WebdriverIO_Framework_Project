...
Feature: Retirement Planning Calculator
  ...
  Scenario Outline: Calculating Retirement Amount - Happy Path
    ...
    When I select Social Security benefits radio button as "<option>"
    Then The Social Security benefits radio button "<selected_Option>" should be selected
    And I select marital status as "<married>"
    And I enter social Security Override Amount as "social_security_override"
